...
class PointCloudService : public QObject
{
  ...
  public:
	...
	// 执行工件检测（供PartInspectDialog直接调用）
	// 返回值：true表示检测成功，false表示失败
	// 检测结果通过getPartInspectResult()获取
	...
	bool executeElectrodeInspect(const QString& partType, const QString& electrodeType, const QString& rfid);
  ...
  public:
    ...
    bool               icpInternal(const QJsonObject& params,double &finalRms, QString* errorMessage, ccGLMatrix* transMat = nullptr); // 内部ICP配准函数
	...
	static Eigen::Matrix4d transformToZeroPose(double B_deg, double C_deg, const Eigen::Vector3d& pivot_B, const Eigen::Vector3d& pivot_C);
