...
void qTcpPlugin::setMainAppInterface(ccMainAppInterface* app)
{
	...
	m_logDock = new LogDockWidget((QWidget*)mainWin);
...
QList<QAction*> qTcpPlugin::getActions()
{
	...
	if (!m_calibrationResultAction)
	{
		m_calibrationResultAction = new QAction(QIcon(":/CC/plugin/qTcpPlugin/res/result.png"), "标定结果", this);
...
{
	PartElectrodeConfigDialog dialog(m_app, m_pointCloudService, nullptr);
	dialog.exec();
}
