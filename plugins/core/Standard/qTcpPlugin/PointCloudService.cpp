...
void PointCloudService::saveElectrodeInspectResult(const QString& rfid, const QJsonObject& result) {
...
void PointCloudService::partInspectFunc(const QJsonObject& params)
{
    ...
    // 直接调用 executePartInspect 执行工件检测逻辑，避免重复代码
...
bool PointCloudService::executeCalibration(const QVector<QVector3D>& positions, CalibrationProgressCallback progressCallback)
{
	...
	if (!probeNc.exists())
	{
		...
		QJsonObject obj;
		obj["Result"]                                  = "NG";
		obj["Ret_Err"]                                 = "Probe ball inspection program not found: " ;
		m_cameraCalibrationResult["CalibrationResult"] = obj;
		saveCalibrationStatus();
		return false;
	}
	...
	QString cncFile   = BALL_INSPECT_RESULT_FILE_NAME;
	QString localFile = appDir + "/Temp/ProbeBall.res";
...

{
	...
	if (config.contains("modelFile"))
	{
		...
		loadParams["path"] = QString("%1/Mock/%2").arg(appDir).arg(modelFile);
...
bool PointCloudService::executePartInspect(const QString& partType, const QString& rfid)
{
	...
	if (!clearDbInternal(nullptr, ""))
	{
		...
		QJsonObject obj;
		obj["Result"] = "NG";
		obj["Ret_Err"] = QString("Failed to clear DB before inspection");
		result["InspectResult"] = obj;
		savePartInspectResult(rfid, result);
		m_Status = MachineStatus::Idle;
		return false;
